...
def get_bible_questions_from_llm(model, bible_text, max_retries=1, number_of_questions=1):
        ...
        try:
            if model is not None and bible_text != "":
                ...
                if response.choices:
                    ...
                    print("llm_calls klaida: Atsakymo variantų nerasta.")
...
def generate_questions(models, bible_text, chapter_name, question_file):
    ...
    for model in models:
        ...
        raw_question = get_bible_questions_from_llm(model=model, bible_text=bible_text, number_of_questions=number_of_questions)
        
        if raw_question is None:
            print("llm_calls klaida: tuščias tekstas iš modelio.")
            continue
        
        parsed_list = parser.parse_questions_to_json(raw_question)
        if(parsed_list == ""):
            print(f"llm_calls klaida: nepavyko išparsinti klausimų iš modelio {model}.")
            print("Ar tęsti? (y/n):")
            user_input = input().strip().lower()
            if user_input == 'n':
                sys.exit("Vartotojas nutraukė vykdymą dėl klaidos.")
            else:
                continue
